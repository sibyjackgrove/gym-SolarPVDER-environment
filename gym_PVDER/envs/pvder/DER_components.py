...
class PV_module(object):
    ...
    def fit_MPP_poly(self):
        ...
        utility_functions.print_to_terminal('{}:Found polynomial for MPP :{:.4f}x^3 + {:.4f}x^2 +{:.4f}x^1 + {:.4f}!'.format(self.name,self.z[0],self.z[1],self.z[2], self.z[3]))
...
class SolarPV_DER(PV_module,Grid):
    ...
    controller_parameters = {'50':{'scale_Kp_GCC':0.5,'scale_Ki_GCC':0.1,\
    ...
    def __init__(self,events,grid_model=None,\
        ...
        #Display status
        print('{}:{} kVA instance was succesfully created!'.format(self.name,str(self.Sinverter_rated/1e3)))
    ...
    def check_PV_DER_parameters(self):
        ...
        if _del_I1max_actual > _del_I1max:   #Check if ripple current is less than 10 %
            logging.debug('{}:Filter inductance {:.5} H is acceptable since AC side current ripple is {:.2}% (< 10%)'.format(self.name,_Lf_min,_del_I1max_actual/self.Iarated))
        else:
            logging.debug('{}:Warning:Filter inductance {:.5} H results in AC side current ripple of {:.2}% (> 10%)'.format(self.name,_Lf_min,_del_I1max_actual/self.Iarated))
        
        _I_ripple = (0.25*self.Vdcrated)/(self.Lf_actual*self.fswitching)  #Maximum ripple voltage (p-p) at DC link
        _V_ripple_actual = self.Vdcrated/(32*self.Lf_actual*self.C_actual*(self.fswitching**2))  #Maximum ripple voltage (p-p) at DC link
        _V_ripple_percentage = (_V_ripple_actual/self.Vdcrated)*100
        if _V_ripple_percentage <= 1.0:   #Check if voltage ripple on DC link is less than 1%
            logging.debug('{}:DC link capacitance of {} F is acceptable since voltage ripple is only {:.2}% (< 1%)'.format(self.name,self.C_actual,_V_ripple_percentage))
           ...
           #warnings.warn('Warning:DC link capacitance of {} F results in DC link voltage ripple of {:.3}% (> 1%)!Please use at least {} F.'.format(self.C_actual,_V_ripple_percentage,_C))
            logging.debug('{}:Warning:DC link capacitance of {} F results in DC link voltage ripple of {:.2}% (> 1%)!Please use at least {} F.'.format(self.name,self.C_actual,_V_ripple_percentage,_C))
    ...
    def MPP_table(self):
        ...
        if self.USE_POLYNOMIAL_MPP == True:
            ...
            logging.warning('{}:MPP table not available, using rated DC link voltage!')
    ...
    def ODE_model(self,y,t):
        ...
        elif self.VAR_EXTERNAL:
            self.Q_ref = self.Q_ref
        else:
            self.Q_ref = self.Q_ref_constant 
