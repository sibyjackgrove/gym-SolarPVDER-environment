...
def limit_complex(z,z1,z2,z3):
    """Check if complex number is within limits."""
    r,phi = cmath.polar(z)
    r1,phi = cmath.polar(z1)
    r2,phi = cmath.polar(z2)
    r3,phi = cmath.polar(z3)
    #r = min(max(r,low_limit),high_limit)
    if r1>1.0 or r2>1.0 or r3>1.0:
        #print('Duty cycle limit breached by {} for {}'.format(r,z))
        r_max = max(abs(z1),abs(z2),abs(z3)) #Max duty cycle magnitude
        #r_max = math.sqrt((pow(abs(z1),2)+pow(abs(z2),2)+pow(abs(z3),2))/3.0)
        
        del_r_fraction = (r_max-1)/r_max
        
        z = z*(1-del_r_fraction)
        #print('New duty cycle calculated:',z)
